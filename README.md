...
```
Now you can download nginx configuration files avaible in this repoand apply new configurations:
``` bash
...
stream {
    ...
    server {
        listen x.x.x.x:2049;   #insert here your ip address
        proxy_pass $upstream2049;
    }

    server {
        listen x.x.x.x:111;   #insert here your ip address
        proxy_pass $upstream111;
    }

    server {
        listen x.x.x.x:20048;  #insert here your ip address
...
cd /etc/nginx/conf
sudo touch upstream20048.conf upstream2049.conf upstream111.conf upstreams.conf
cd /etc/nginx/record
...
| /apiproxy/rproxy/forward | it return current running forward |
...
there is only one post request:

| request path | body | request scope |
| --- | --- | --- |
| /apiproxy/rproxy/forward | {"src" : "\*src address\*","dest" : "\*dest address\*","client" : "\*client name\*"} | it create a new stream forward, ir require ip address to redirect, the forward destination and the client name.  <br>the api update the nginx config file for redirect nfs ports (2049,20048,111) and the custom record, next it restart nginx service.  <br>if you use apik3s is recommended use for label "client" the same name of client workspace. |

## DELETE Request

there is only one delete request in this api.

| request path | request scope |
| --- | --- |
| /apiproxy/rproxy/forward/:client/:src/:dest | it delete an stream forward.  <br>it delete every map source address occurs,and remove this specif forward by the custom record.  <br>if the destination address is never used after changes it remove upstream to this address. |
